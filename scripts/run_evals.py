...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run end-to-end desktop agent evaluations."
        )
    )
...
def run_worker(
    ...
    with tempfile.TemporaryDirectory(
        ...
        case_path.write_text(
            loaded_case.case.model_dump_json(
                indent=2
            ),
            encoding="utf-8",
        )
        ...
        if output_path.exists():
            ...
            payload = {
                ...
                "error": {
                    ...
                    "message": (
                        "Evaluation worker did not "
                        "produce an output file."
                    ),
                    "traceback": (
                        process_output or None
                    ),
                },
            }
...
def build_case_result(
    ...
    execution_error = worker_payload.get(
        "error"
    )
    ...
    if execution_error is not None:
        return EvalCaseRunResult(
            ...
            description=(
                loaded_case.case.description
            ),
            source_path=(
                loaded_case.source_path
            ),
            line_number=(
                loaded_case.line_number
            ),
            passed=False,
            score=0.0,
            ...
            error=(
                EvalExecutionError.model_validate(
                    execution_error
                )
            ),
        )
...
def main() -> int:
    ...
    if not loaded_cases:
        print(
            "No evaluation cases were selected."
        )
        return 1
